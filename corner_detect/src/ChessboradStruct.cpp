	...
	for (int k = 0; k < chessboards.size(); k++)
	{
		...
		for (int i = 0; i < chessboards[k].rows; i++)
			for (int j = 0; j < chessboards[k].cols; j++)
			{
				...
				int idx = i * chessboards[k].cols + j;
				cv::Point2f pt(corners.p[d].x + rect.x, corners.p[d].y + rect.y);
				cv::circle(disp, pt, n, s, n);
				cv::putText(disp, std::to_string(idx), pt + cv::Point2f(5, -5), 
